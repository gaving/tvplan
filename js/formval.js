                         ...
                         reqd)   // true if required
{
  ...
  if (!email2.test(tfld))
    msg (ifld, "warn", "(warning: unusual e-mail address!)");
  else
    msg (ifld, "warn", "");
  return true;
};
                         ...
                         reqd)   // true if required
{
  ...
  var pfld = trim(vfld.value);
  var passRE = /^(?=.*[a-z])\w{6,}$/
  if (!passRE.test(pfld)) {
    msg (ifld, "error", "(error: invalid password!)");
    setfocus(vfld);
    return false;
  }
  else
...
function validateConfirmPassword    (vfld,   // element to be validated
                         ...
                         reqd)   // true if required
{
 ...
 if (vfld.value != vfld2.value) {
    msg (ifld, "error", "(error: passwords don't match!)");
    setfocus(vfld);
    return false;
  }
  else
